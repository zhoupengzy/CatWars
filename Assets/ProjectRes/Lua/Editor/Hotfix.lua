...
function hotfix(filename)
    ...
    update_table(newModule, oldModule,updated_tables,1)
...
function update_table(new_table, old_table, updated_tables,depth)
    ...
    for key, value in pairs(new_table) do
        ...
        elseif type_value == "table" then
            if ( updated_tables[value] == nil ) then
                ...
                update_table(value, old_value,updated_tables,depth+1)
            end
        end
    end
    ...
    if type(old_meta) == "table" and type(new_meta) == "table" then
        update_table(new_meta, old_meta,updated_tables,depth+1)
    end
end